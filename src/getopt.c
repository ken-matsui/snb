...
is_option(char* argv_element, int only) {
  return (
      (argv_element == NULL) || (argv_element[0] == '-')
      || (only && argv_element[0] == '+')
  );
}
...
static int
getopt_internal(
    int argc, char** argv, char* shortopts, GETOPT_LONG_OPTION_T* longopts,
    int* longind, int only
) {
  ...
  if (longopts != NULL
      && (memcmp(argv[optind], "--", 2) == 0 || (only && argv[optind][0] == '+')
      )
      && optwhere == 1) {
    /* handle long options */
          ...
          if (longopt_match < 0)
            ...
            if (opterr)
              fprintf(
                  ...
                  "(could be `--%s' or `--%s')\n",
                  argv[0], argv[optind], longopts[longopt_match].name,
  ...
  if (longopt_match < 0 && shortopts != NULL) {
    ...
    has_arg =
        ((cp[1] == ':') ? ((cp[2] == ':') ? OPTIONAL_ARG : required_argument)
                        : no_argument);
...
int
getopt_long(
    int argc, char** argv, const char* shortopts,
    const GETOPT_LONG_OPTION_T* longopts, int* longind
) {
  return getopt_internal(
      argc, argv, (char*)shortopts, (GETOPT_LONG_OPTION_T*)longopts, longind, 0
  );
}
...
getopt_long_only(
  ...
  return getopt_internal(
      argc, argv, (char*)shortopts, (GETOPT_LONG_OPTION_T*)longopts, longind, 1
  );
}
