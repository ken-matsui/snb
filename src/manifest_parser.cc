...
bool
ManifestParser::ParsePool(std::string* err) {
  std::string name;
...
bool
ManifestParser::ParseRule(std::string* err) {
  std::string name;
  if (!lexer_.ReadIdent(&name))
...
bool
ManifestParser::ParseDefault(std::string* err) {
  EvalString eval;
...
bool
ManifestParser::ParseEdge(std::string* err) {
...
bool
ManifestParser::ParseFileInclude(bool new_scope, std::string* err) {
  EvalString eval;
  if (!lexer_.ReadPath(&eval, err))
    return false;
  std::string path = eval.Evaluate(env_);
