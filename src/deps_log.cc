...
bool
DepsLog::OpenForWrite(const std::string& path, std::string* err) {
...
bool
DepsLog::RecordDeps(
    Node* node, TimeStamp mtime, const std::vector<Node*>& nodes
) {
  return RecordDeps(
...
LoadStatus
DepsLog::Load(const std::string& path, State* state, std::string* err) {
...
bool
DepsLog::Recompact(const std::string& path, std::string* err) {
