...
bool
InternalGetFullPathName(
    const StringPiece& file_name, char* buffer, size_t buffer_length,
    string* err
) {
  DWORD result_size = GetFullPathNameA(
      file_name.AsString().c_str(), buffer_length, buffer, nullptr
  );
  if (result_size == 0) {
...
bool
SameDriveFast(StringPiece a, StringPiece b) {
...
// Return true if paths a and b are on the same Windows drive.
bool
SameDrive(StringPiece a, StringPiece b, string* err) {
...
string
IncludesNormalize::AbsPath(StringPiece s, string* err) {
  if (IsFullPathName(s)) {
...
bool
IncludesNormalize::Normalize(const string& input, string* result, string* err)
    const {
