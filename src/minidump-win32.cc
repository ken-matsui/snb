...
)(IN HANDLE, IN DWORD, IN HANDLE, IN MINIDUMP_TYPE,
...
void
CreateWin32MiniDump(_EXCEPTION_POINTERS* pep) {
  ...
  if (hFile == NULL) {
    Error(
        "failed to create minidump: CreateFileA(%s): %s", temp_file,
        GetLastErrorString().c_str()
    );
    return;
  }
  ...
  BOOL rv = mini_dump_write_dump(
      GetCurrentProcess(), GetCurrentProcessId(), hFile, mdt,
      (pep != 0) ? &mdei : 0, 0, 0
  );
  CloseHandle(hFile);
