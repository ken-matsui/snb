...
struct DepfileParserTest : public testing::Test {
  bool
  Parse(const char* input, std::string* err);
...
bool
DepfileParserTest::Parse(const char* input, std::string* err) {
  input_ = input;
...
TEST_F(DepfileParserTest, Basic) {
  ...
  EXPECT_EQ("build/ninja.o", parser_.outs_[0]);
...
TEST_F(DepfileParserTest, Continuation) {
  ...
  ASSERT_EQ("", err);
  ASSERT_EQ(1u, parser_.outs_.size());
  EXPECT_EQ("foo.o", parser_.outs_[0]);
...
TEST_F(DepfileParserTest, BackSlashes) {
  ...
  ASSERT_EQ("", err);
  ASSERT_EQ(1u, parser_.outs_.size());
  EXPECT_EQ("Project\\Dir\\Build\\Release8\\Foo\\Foo.res", parser_.outs_[0]);
  EXPECT_EQ(4u, parser_.ins_.size());
}
...
TEST_F(DepfileParserTest, Spaces) {
  ...
  ASSERT_EQ("", err);
  ASSERT_EQ(1u, parser_.outs_.size());
  EXPECT_EQ("a bc def", parser_.outs_[0]);
...
TEST_F(DepfileParserTest, MultipleBackslashes) {
  ...
  ASSERT_EQ("", err);
  ASSERT_EQ(1u, parser_.outs_.size());
  EXPECT_EQ("a b#c.h", parser_.outs_[0]);
  ASSERT_EQ(3u, parser_.ins_.size());
  EXPECT_EQ("\\\\ ", parser_.ins_[0]);
  EXPECT_EQ("\\\\\\\\", parser_.ins_[1]);
  EXPECT_EQ("\\\\share\\info\\#1", parser_.ins_[2]);
}
...
TEST_F(DepfileParserTest, EscapedColons) {
  ...
  ASSERT_EQ("", err);
  ASSERT_EQ(1u, parser_.outs_.size());
  EXPECT_EQ("c:\\gcc\\x86_64-w64-mingw32\\include\\stddef.o", parser_.outs_[0]);
  ASSERT_EQ(1u, parser_.ins_.size());
  EXPECT_EQ("c:\\gcc\\x86_64-w64-mingw32\\include\\stddef.h", parser_.ins_[0]);
}
...
TEST_F(DepfileParserTest, EscapedTargetColon) {
  ...
  ASSERT_EQ("", err);
  ASSERT_EQ(1u, parser_.outs_.size());
  EXPECT_EQ("foo1\\", parser_.outs_[0]);
  ASSERT_EQ(1u, parser_.ins_.size());
...
TEST_F(DepfileParserTest, SpecialChars) {
  ...
  ASSERT_EQ("", err);
  ASSERT_EQ(1u, parser_.outs_.size());
  EXPECT_EQ("C:/Program Files (x86)/Microsoft crtdefs.h", parser_.outs_[0]);
  ASSERT_EQ(5u, parser_.ins_.size());
  EXPECT_EQ("en@quot.header~", parser_.ins_[0]);
  EXPECT_EQ("t+t-x!=1", parser_.ins_[1]);
  EXPECT_EQ(
      "openldap/slapd.d/cn=config/cn=schema/cn={0}core.ldif", parser_.ins_[2]
  );
  EXPECT_EQ("Fu\303\244ball", parser_.ins_[3]);
  EXPECT_EQ("a[1]b@2%c", parser_.ins_[4]);
}
