...
TEST_F(ParserTest, Empty) { ASSERT_NO_FATAL_FAILURE(AssertParse("")); }
...
TEST_F(ParserTest, PhonySelfReferenceIgnored) {
  ASSERT_NO_FATAL_FAILURE(AssertParse("build a: phony a\n"));
...
TEST_F(ParserTest, PhonySelfReferenceKept) {
  const char kInput[] = "build a: phony a\n";
  ManifestParserOptions parser_opts;
