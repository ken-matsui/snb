...
struct Env {
  virtual ~Env() {}
  virtual std::string
  LookupVariable(const std::string& var) = 0;
};
...
struct EvalString {
  ...
  void
  AddSpecial(std::string_view text);
...
struct Rule {
  ...
  void
  AddBinding(const std::string& key, const EvalString& val);

  static bool
  IsReservedBinding(const std::string& var);
...
struct BindingEnv : public Env {
  ...
  const Rule*
  LookupRule(const std::string& rule_name);
  const Rule*
  LookupRuleCurrentScope(const std::string& rule_name);
  const std::map<std::string, const Rule*>&
  GetRules() const;

  void
  AddBinding(const std::string& key, const std::string& val);
  ...
  std::string
  LookupWithFallback(const std::string& var, const EvalString* eval, Env* env);
